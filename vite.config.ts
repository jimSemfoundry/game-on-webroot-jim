      ...
      return {
        ...
        transform(code: string, id: string) {
          ...
          if (id.endsWith('.ts') || id.endsWith('.tsx') || id.endsWith('.js') || id.endsWith('.jsx')) {
            ...
            transformedCode = transformedCode.replace(/(['"`])\/images\//g, `$1${baseUrl}images/`);
            transformedCode = transformedCode.replace(/(['"`])\/locales\//g, `$1${baseUrl}locales/`);
            transformedCode = transformedCode.replace(/(['"`])\/icons\//g, `$1${baseUrl}icons/`);
            transformedCode = transformedCode.replace(/(['"`])\/logo\//g, `$1${baseUrl}logo/`);
          ...
          if (id.endsWith('.css') || id.endsWith('.scss') || id.endsWith('.sass')) {
            let transformedCode = code;
            // 替换 CSS 中的 url() 引用
            transformedCode = transformedCode.replace(/url\(\/images\//g, `url(${baseUrl}images/`);
            transformedCode = transformedCode.replace(/url\(\/locales\//g, `url(${baseUrl}locales/`);
            transformedCode = transformedCode.replace(/url\(\/icons\//g, `url(${baseUrl}icons/`);
            transformedCode = transformedCode.replace(/url\(\/logo\//g, `url(${baseUrl}logo/`);
            return transformedCode;
          }
          return code;
        },
