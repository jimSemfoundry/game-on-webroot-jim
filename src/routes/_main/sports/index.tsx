...
const BETBY_SDK_URL = import.meta.env.VITE_BETBY_SDK_URL || DEFAULT_BETBY_SDK_URL;
...
const RegionRestrictionError = () => {
  ...
  return (
    <div className="h-full text-sm font-bold text-warning flex items-center justify-center">
...
export const Route = createFileRoute("/_main/sports/")({
  ...
  loader: async () => {
    try {
      // 使用 publicService.getBaseConfig() 检查 is_show_betby
      const data = await publicService.getBaseConfig();

      // 如果 is_show_betby === 0，抛出错误显示地区不支持
      if (data?.data?.is_show_betby === 0) {
      ...
      // 如果是地区限制错误，重新抛出
      if (error instanceof Error && error.message === "REGION_NOT_SUPPORTED") {
        throw error;
      }
      return { data: { is_show_betby: 1 } }; // 默认允许访问
    }
  },
  errorComponent: RegionRestrictionError
});
  ...
  return (
    ...
    <SportsBonusGuard>
      <div className="w-full h-full min-h-screen relative">
        {/* Betby 容器 */}
        {!isRestricted && <div id="betby" className="pb-[calc(4.5rem+var(--safe-area-inset-bottom))]" />}
        ...
        {showLoading && (
          <div className="absolute inset-0 flex items-center justify-center bg-base-300 z-50">
            <div className="flex flex-col items-center gap-2">
              ...
              <span
                className="text-base-content font-bold text-base sm:text-lg uppercase whitespace-break-spaces">{t("common:common.challengeEverything")}</span>
            </div>
          </div>
        )}
        ...
        {isRestricted && (
          <div className="absolute inset-0 flex items-center justify-center px-6 bg-base-300">
            <div className="flex w-full max-w-sm flex-col items-center gap-4 text-center">
              ...
              <img src={`${getPathInROIBEST()}/icons/isometric/23.svg`} alt="userRestriction" className="h-20 w-20" />
              <button
