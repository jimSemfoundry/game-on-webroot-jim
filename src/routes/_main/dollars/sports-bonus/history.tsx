...
import { ESport, InnerBonusSlogan, InnerSportsContainer } from "@/sections/dollars/components.tsx";
...
export const Route = createFileRoute("/_main/dollars/sports-bonus/history" as any)({
  component: History
});
        ...
        <div className="relative min-h-[240px]">
          <div className="space-y-2">
            {status.data.map((row: Record<string, any>, index: number) => {
              ...
              // promo 路径领取的 wallet 用户没付钱,不展示 Buy Amount,改展示 Promo Code + grant_amount(USDT)。
              // 兼顾数据兜底:fix 之前写入的老 promo record 缺 purchase_amount,会导致 Decimal(undefined) 抛错。
              const isPromo = parsed_data?.grant_type === "promo_code";
              const grant_amount_fiat = formatCurrency({
                amount: convertCurrency({
                  amount: parsed_data?.grant_amount || 0,
                  fromCurrency: "USDT",
              ...
              return (
                ...
                >
                  <InnerItemWrap
                    label={t("bonus:sportsBonusStore")}
                      ...
                      value={
                        <div className="flex flex-col items-end">
                          <span className="text-primary font-bold">
                            {Decimal(parsed_data?.purchase_amount ?? 0).toDP(18, Decimal.ROUND_DOWN).toString()}{" "}
                  ...
                  {isPromo && (
                    <InnerItemWrap
                      label={t("bonus:promo_code")}
                      value={
                        <div className="flex flex-col items-end">
                          <span className="text-primary font-bold">
                            {Decimal(parsed_data?.grant_amount ?? 0).toDP(18, Decimal.ROUND_DOWN).toString()}{" "}
                            USDT
                          </span>
                          <span>≈{grant_amount_fiat}</span>
                        </div>
                      }
                    />
                  )}
                  ...
                  <InnerItemWrap
                    ...
                    value={
                      <div className="flex flex-col items-end">
                        <span className="text-primary font-bold">{wager_require}</span>
                      </div>
                    }
                  />
          ...
          {!isFetching && Number(data?.total || 0) === 0 && (
            <NothingFound icon={"/images/bonus/sport.png"} />
          )}
        </div>
...
const InnerStatusWrap = ({ value }: { value: string }) => {
  ...
  return (
    <div className={clsx("text-xs border rounded-lg px-1 py-0.5", cfg.style)}>
