...
export const CurrencySelector: React.FC<CurrencySelectorProps> = ({
  ...
  const [hideZeroBalances, setHideZeroBalances] = useState(() => {
    const saved = localStorage.getItem("wallet-hideZeroBalances");
  ...
  useEffect(() => {
    localStorage.setItem("wallet-hideZeroBalances", JSON.stringify(hideZeroBalances));
  }, [hideZeroBalances]);
      ...
      <div key={groupName} className="">
        ...
        <div className={`space-y-1 rounded-field p-3 bg-base-300 ${!isDesktop && showGroupHeader ? "mt-1" : ""}`}>
          {currencies.map((currency: any) => {
            ...
            const isCrypto = currency.currency_type === "CRYPTO";
                ...
                <div className="flex items-center justify-between w-full">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setModalOpen(false)
                          openUserFinanceModalWithTab("swap");
                        }}
                      >
                        SWAP
                      </button>
                  ...
                  {showBalance && (
                    <div className="text-right">
                      {isCrypto && displayAmount ? (
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold text-base-content">{displayAmount}</span>
                          <span className="text-xs text-base-content/50">{formatAmount(balance, currency.currency)}</span>
                        </div>
                      ) : (
  ...
  const defaultTrigger = (
    <button
      className={`flex items-center flex-1 px-2 min-w-28 text-base-content/50 md:text-md font-semibold text-sm hover:text-base-content/70 transition-colors cursor-pointer ${className}`}
      onClick={handleCurrencySwitchClick}
    >
      <CurrencyIcon currency={selectedCurrency} className="w-5 h-5 md:w-6 md:h-6 mr-2" />
      {isBalanceLoading || isCurrencyLoading ? (
        ...
        <div className="flex flex-col text-left min-w-0">
          {(() => {
            ...
            if (isCrypto && displayAmount) {
              return (
                <>
                  <span className="text-xs font-semibold text-base-content truncate">{displayAmount}</span>
      ...
      <ChevronDown
        className={`w-4 h-4 ltr:ml-auto rtl:mr-auto transition-transform ${dropdownOpen ? "rotate-180" : ""} flex-shrink-0`}
      />
    </button>
  );
    ...
    <>
      <div className="relative" ref={dropdownRef}>
        ...
        {dropdownOpen && !isLoading && !isCurrencyLoading && (
          <div className="absolute p-3 top-full right-0 mt-2 bg-base-400 rounded-lg shadow-xl z-50 min-w-[420px] min-h-[514px] overflow-hidden">
            ...
            <div>
              ...
              <div className="mb-2 flex items-center gap-2">
                <label className="input input-ghost input-md bg-base-300 focus-within:bg-base-300 focus-within:outline-none flex-1 rounded-lg flex items-center gap-2">
              ...
              <div className="bg-base-300 absolute bottom-0 left-0 right-0 h-22 px-5">
                <div className="flex items-center justify-between w-full h-full">
                  ...
                  >
                    ...
                    <p className="font-bold text-base">{currentDisplayCurrencyData?.display_name || currentDisplayCurrency}</p>
      ...
      >
        <div className="flex flex-col h-[75vh] md:h-[460px] max-h-[75vh] pb-[130px]">
          ...
          <div className="flex-shrink-0 pb-2">
            <div className="flex items-center gap-2">
              <label className="input bg-base-300 input-ghost focus-within:bg-base-300 focus-within:!outline-none focus:!outline-none flex-1 rounded-field">
