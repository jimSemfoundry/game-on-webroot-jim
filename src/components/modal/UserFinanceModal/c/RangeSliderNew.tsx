...
export const RangeSliderNew = () => {
  ...
  const minAmountValue = useMemo(() => {
    ...
    return depositFiat.method?.min;

  }, [depositFiat.method?.min, depositFiat.currency?.currency, setDepositFiat, currentPromo]);
  ...
  const maxAmountValue = useMemo(() => {
    if (currentPromo?.promo_type === 1) {
      ...
      const maxAmountValue = valueNum === 0 ? depositFiat.method?.max : Math.min(depositFiat.method?.max, valueNum);
      return maxAmountValue;
    }
    return depositFiat.method?.max;
  }, [depositFiat.method?.max, depositFiat.currency?.currency, setDepositFiat, currentPromo]);
  ...
  const rangeLabels = useMemo(() => {
    ...
    // 兜底：如果最大值小于等于最小值，至少给一个点
    if (maxAmountValue <= minAmountValue) {
      labels.push({ value: 0, label: t('finance:min'), amount: minAmountValue });
      return labels;
    }

    const segments = 5; // 把区间分成 4 段，得到 5 个点
    const step = (maxAmountValue - minAmountValue) / segments;

    for (let i = 0; i <= segments; i++) {
      const amount = minAmountValue + step * i;
    ...
    return labels;
  }, [minAmountValue, maxAmountValue, t]);
  ...
  return (
    <>
      ...
      {!(currentPromo?.promo_code === 'special_offer_thursday' || currentPromo?.promo_code === 'special_offer_sunday') && (
      ...
      {(currentPromo?.promo_code === 'special_offer_thursday' || currentPromo?.promo_code === 'special_offer_sunday') && (
        <p className="text-sm text-warning font-semibold leading-3 mt-2">
          {t('finance:maximum_deposit_for_bonus_eligibility',
            { value: ` ${maxAmountValue} ${depositFiat.currency?.currency}` })
          }
        </p>
      )}
    </>
