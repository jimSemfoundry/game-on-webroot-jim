...
export default function MiniSlotBonusModal(
  {
    open,
    onClose
  }: {
  ...
  const match = parser((bonusConfig?.data ?? []).find((bonus: Record<string, any>) => bonus?.name?.includes(EBonus.MINI))?.extra_data);
    ...
    >
      <section className={"text-xs font-semibold"}>
        ...
        <div className={"whitespace-pre-line text-base-content/50 mt-4"}>
          <Trans i18nKey={"bonus:bonus_rules_desc.mini"} />
          ...
          <div className={"mt-4"}>
            <Trans i18nKey={"bonus:bonus_rules_desc.approximately"} values={{ value: "15%" }}
                   components={[<span className={"text-primary"} />]} /></div>
        </div>

        {/* 非数据描述 */}
        <InnerDescription hideId={0} bonusKey={EBonus.MINI} currency={EBonus.TOKEN} className={"!p-0 mt-4"} />
      </section>
    </Modal>
  );
}