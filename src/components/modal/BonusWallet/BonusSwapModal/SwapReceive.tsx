...
import { InnerExchange } from "@/components/modal/BonusWallet/BonusSwapModal/SwapSend.tsx";
    ...
    <div className="flex flex-col gap-2">
      <div className="bg-base-300 p-3 flex items-center justify-between rounded-xl gap-2">
        ...
        <div className="flex-shrink-0 flex flex-col gap-2 items-end">
          <div className="flex items-center gap-2 w-[150px] sm:w-[230px] rounded-full p-2 bg-base-400">
            <img src="/images/dollars/bonus.png" className={"w-6 h-6"} alt="" />
...
const InnerCurrency = ({ rate }: { rate: string }) => {
  ...
  const fiat = useMemo(() => {
    const v = ["USDC", "USDT"].includes(bonusSwapTo.currency?.currency) ? "USD" : bonusSwapTo.currency?.currency;
...
const InnerDataLabel = ({ label, amount, currency, decimal }: {
  label: string,
  amount: string,
  currency: string,
  decimal: number
}) => {
  return <div className="flex items-center justify-between mt-1">
    ...
    <div className="text-right">
      <div className="text-base-content text-xs font-bold flex items-center gap-1 justify-end">
        {/*<CurrencyIconPlaceholder currency={currency} />*/}
