...
import { EBonus, InnerDescription, InnerHeader, InnerLabel } from "@/sections/dollars/components.tsx";
    ...
    >
      <section className={"text-xs font-semibold"}>
        ...
        <div className={"grid grid-cols-2 gap-2"}>
          ...
          <InnerLabel title={t("bonus:periodComplete")}
        ...
        <div className={"whitespace-pre-line text-base-content/50 mt-4"}>
          <Trans
            i18nKey={"bonus:bonusGameplay"} />

          <div className="bg-base-200 rounded-field p-3 mt-4 flex flex-col gap-4">
            {t("bonus:maximumCashout")}
            <span
              className={"text-sm font-extrabold text-primary"}>{t("bonus:bonusObtained", { value: claim_max_multiplier })}
            </span>
        ...
        {/* 非数据描述 */}
        <InnerDescription hideId={0} data={{ ...data, extra_data: "" }} currency={EBonus.TOKEN} className={"!p-0 mt-4"} />
      </section>
    </Modal>
  );
}