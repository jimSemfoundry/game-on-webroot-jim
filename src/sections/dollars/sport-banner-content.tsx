import { useCallback, useEffect, useRef, useState } from "react";
import dayjs from "dayjs";
import { bonus_dollars_router_path } from "@/sections/dollars/bonus-wallet.tsx";
import { InnerConfirmBox } from "@/sections/dollars/components.tsx";
import { useTranslation } from "react-i18next";
import styled from "styled-components";
import { emitter } from "@/store/emitter.ts";
import { InnerContentVisible } from "@/components/header/message-v2/c/InnerComponents.tsx";
import { parser } from "@/components/modal/UserFinanceModal/c/WithdrawMethodInfoAddModal.tsx";
import {
  InnerBannerTitle,
  InnerBonusTips,
  InnerContainer,
  InnerTimeDesc,
  InnerTimeLabel,
  useNavigateGuard
} from "@/sections/casino/hero-banner/InnerComponents.tsx";
import duration from "dayjs/plugin/duration";

dayjs.extend(duration);

const init = [0, 0, 0, 0];

export const DollarsSportBannerItem = ({ data, content }: {
  data: Record<string, any>
  content: string
}) => {
  const timerRef = useRef<any>(null);

  const { navigate } = useNavigateGuard();

  const { t } = useTranslation();

  const banner = parser(content);
  const expired_at = data?.extra_data?.expired_at ?? 0;

  const [_timeFinished, setTimeFinished] = useState<boolean>(false);
  const [estimatedTime, setEstimatedTime] = useState<number[]>(init);

  // 钱包数据不是实时更新，有些情况下需要自己监听过期时间--start
  const stopTimer = () => {
    if (timerRef.current) {
      clearInterval(timerRef.current);
      setEstimatedTime(() => init);
    }
    timerRef.current = null;
  };

  const startTimer = useCallback(() => {
    timerRef.current = setInterval(function() {
      const current = dayjs().valueOf();

      const diff = dayjs(expired_at * 1000).diff(current);

      if (diff <= 0) {
        stopTimer();
        setTimeFinished(true);
        emitter.emit("UPDATE", "SPORT");
        return;
      }

      const duration = dayjs.duration?.(diff);
      const days = Math.floor(duration.asDays());
      const hours = Math.floor(duration.asHours()) % 24;
      const minutes = Math.floor(duration.asMinutes()) % 60;
      const seconds = Math.floor(duration.asSeconds()) % 60;

      setEstimatedTime(() => [
        days,
        hours,
        minutes,
        seconds
      ]);
    }, 1_000);
  }, [data?.expired_at]);

  // bonus wallet 的内部开关 - 倒计时
  useEffect(() => {
    if (!timerRef.current && expired_at) {
      startTimer();
    }
    return () => {
      stopTimer();
    };
  }, [expired_at]);
  // 钱包数据不是实时更新，有些情况下需要自己监听过期时间--end

  return (<div className={"bg-base-200 rounded-xl w-full h-[209px] sm:h-[258px]"}>
    <InnerSportContainer src={banner?.float_image_list?.[0]?.mobile_image} className={"p-4 h-full"}>
      {/* 引导bonus的tips */}
      <InnerContentVisible show={data?.extra_data?.handle_status === 0}>
        <InnerBonusTips
          icon="/images/dollars/sport.png"
          type={"SPORT"}
          text={t("bonus:sports_bonus")} />
      </InnerContentVisible>

      <div className="flex flex-col gap-2 mt-2">
        {/* 活动标语 */}
        <InnerContentVisible show={data?.extra_data?.handle_status !== 0}>
          {/*TODO: sp_bo 服务端添加数据*/}
          <InnerBannerTitle list={banner?.text_list ?? []} data={data} banner={banner} />
        </InnerContentVisible>

        {/* 倒计时 */}
        <InnerContainer className="relative w-50 rounded-lg p-2 mt-2">
          <InnerTimeDesc text={t("bonus:expires_in")} />

          <div className="flex w-full items-center gap-1">
            <InnerTimeLabel value={estimatedTime[0]} label={t("common:common.daysUnit")} />
            <InnerTimeLabel value={estimatedTime[1]} label={t("common:common.hours")} />
            <InnerTimeLabel value={estimatedTime[2]} label={t("common:common.minutes")} />
            <InnerTimeLabel value={estimatedTime[3]} label={t("common:common.seconds")} />
          </div>

          <InnerConfirmBox
            className={"mt-2"}
            onClick={() => navigate(bonus_dollars_router_path["SPORT"], true)}>
            {t(`banner:${banner?.button_list[0]?.button_text}`)}
          </InnerConfirmBox>
        </InnerContainer>
      </div>
    </InnerSportContainer>
  </div>);
};

const InnerSportContainer = styled.div<{src?: string}>`
    background: url(${props => props?.src}) no-repeat right bottom;
    background-position: right 0px bottom 0px;
    position: relative;
`;