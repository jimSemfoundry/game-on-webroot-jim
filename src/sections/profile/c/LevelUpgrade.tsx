...
export function LevelUpgrade({ onClick }: { onClick: () => void }) {
  ...
  const progress = useMemo(
    () =>
      Decimal(Math.min((userXP / (fullXP || 1)) * 100, 100))
  ...
  return (
    <div className="h-full flex flex-col justify-between z-10">
      <img src={`/images/vip/levels/${status?.vip}.png`} className="absolute rtl:left-3 rtl:right-auto right-3 top-0 -translate-y-1/3 h-18 md:h-35" alt="" />
      <div onClick={onClick}>
        ...
        <SmallLoading
          ...
          content={
            <p className="font-semibold text-xs sm:text-xl text-base-content/50">
      ...
      <div className="flex items-center flex-1 my-4 sm:my-0">
        <progress className="progress progress-primary h-1.5 sm:h-2.5" value={progress} max={100} />
      </div>
      <SmallLoading
        loading={isLoading}
        className={"!bg-base-200 w-30"}
