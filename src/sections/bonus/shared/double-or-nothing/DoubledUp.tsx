...
export const DoubledUp = ({ donData }: { donData: IDoubledUpProps }) => {
        ...
        <div className="h-[356px] w-full flex flex-col items-center justify-center rounded-2xl pt-[90px] relative"
          ...
          <div className="flex items-center gap-2 justify-center mt-4 mb-5">
            ...
            <div className="text-primary text-2xl leading-6 font-bold ">
              {formatWithConversion(new Decimal(donData?.final_amount).mul(2).toNumber(), user?.currency_fiat || 'USDT', { 
                showSymbol: true,
                showCode: true,
          ...
          <button className="btn btn-primary h-12 w-50 " onClick={() => setOpen(false)}>{t('bonus:continue')}</button>
        </div>
      </div>
    </Modal>
  );
