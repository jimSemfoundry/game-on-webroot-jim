...
import { ClaimData } from "./bonus-vip-monday-card";
...
export function MysteryBoxModal({ isOpen, onClose, id }: MysteryBoxModalProps) {
  ...
  const [mysteryBoxData, setMysteryBoxData] = useState<ClaimData | null>(null);
  const { setSyncAction } = useBoundStore();
  const { refetch } = useGetMondayVipBonus();

  const handleOpenBox = async () => {
    if (!id) {
      toast.error('bonus:claim_failed');
      return;
    }
    setIsLoading(true);
    authService.claimMondayVipBonus(id).then((res) => {
      ...
      if (res.code === 200 || res.code === 0) {
        ...
        if (res.code === 4001 || res.code === 50023 || res.code === 50021) {
          ...
          setSyncAction("OPEN_BONUS_CLAIM_RESPONSE_MODAL", {
            code: res.code,
            tryAgain: res.donId ? () => handleOpenBox() : undefined
          });
        }
      }
    }).finally(() => {
    ...
    >
      ...
      <div className="mb-2 text-center z-10">
        <div className="flex flex-col">
          ...
          <h1
            className="text-[70px] font-[Lobster] -mt-17 text-center w-full"
            style={{
              background:
                "linear-gradient(183deg, color(display-p3 0.8246 0.6993 1) 2.12%, color(display-p3 0.9325 0.9206 1) 57.21%, color(display-p3 0.726 0.599 1) 57.33%, color(display-p3 1 0.6815 0.9416) 102.79%)",
              backgroundClip: "text",
  ...
  const renderOpened = (data: ClaimData | null) => (
    <div
      className="flex flex-col items-center sm:h-[600px] relative overflow-hidden"
      style={{
        transform: "translateZ(0)", // 启用硬件加速
  ...
  const renderContent = (data: ClaimData | null) => {
