...
export const SpecialOffersModal = ({ open, onClose }: { open: boolean, onClose: () => void }) => {
  ...
  const { total } = useGetPromoByPage(open);
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      style={{
        height: isMobile ? `calc(100% - var(--safe-area-inset-top) - 48px)` : '770px',
        maxHeight: isMobile ? `calc(100% - var(--safe-area-inset-top) - 48px)` : '770px',
      }}
