...
export const BonusAchievementsListV2 = ({
  ...
  const isMin1550 = useMediaQuery("(min-width: 1550px)");
  const listColumnCount = isMin1550 ? 7 : isMin1150 ? 5 : isMin768 ? 3 : 2;
  ...
  const achievementCards = (data: Achievement[]) => {
    return data.map((achievement) => {
      ...
      const finishedStepCount = achievement.steps.filter((step) => step.is_finish).length;
      const totalStepCount = achievement.steps.length;
      ...
      return (
        <InnerGuardAchievementContainer
          ...
          {layout === "list" && !enableCarousel ? (
            <div
              className={cn(
                "h-[188px] rounded-field border bg-base-200 p-4 py-6 transition-all duration-200 cursor-pointer relative",
            ...
            >
              <div className="absolute top-2 right-2 flex items-center gap-1">
                {totalStepCount === 1 ? (
                  finishedStepCount ?
                    <Iconify
                      icon="custom:achievements_finsh"
                      width={24}
                      height={24}
                  ...
                  <span className="text-xs text-base-content/50 font-semibold">
                    <span className="text-base-content">{finishedStepCount}</span>/{totalStepCount}
                  </span>
                )}
              </div>
              <div className="flex flex-col h-full items-center gap-3 justify-between">
                ...
                <p className="text-xs font-bold text-base-content whitespace-normal break-words text-center leading-tight">{achievement.name}</p>
                {isClaimable ? (
                  <button
                    type="button"
                    className="btn btn-primary btn-md leading-5"
                    onClick={(event) => {
            ...
            <div
              className={cn(
                enableCarousel
                  ? "relative rounded-box cursor-pointer overflow-hidden w-28 md:w-32 h-full"
          ...
          {enableCarousel ? (
            ...
            achievements.length !== 0 && (
              <BonusListHeader
                title={t("common.achievements")}
                icon={
                  <Iconify icon="custom:profile-achievements" className="text-primary" />
                }
                hasArrow={isMobile}
              >
                <div
                  className={cn(
                    layout === "list"
                      ? "grid grid-cols-2 gap-3 pb-2 min-[768px]:grid-cols-3 min-[1150px]:grid-cols-5 min-[1550px]:grid-cols-7"
                      : "grid grid-cols-3 gap-1.5 sm:gap-3 px-2 pb-2",
                    haveMoreButton && !isExpanded && (layout === "list" ? "sm:max-h-[188px]" : "sm:max-h-[220px]"),
                ...
                {haveMoreButton && achievements.length > (layout === "list" ? listColumnCount : 0) && (
                  <div className="hidden justify-center pt-2 sm:flex">
                    <button
                      type="button"
                      className="btn btn-sm capitalize w-[336px] h-12 font-bold text-base-content text-base"
                      onClick={() => setIsExpanded((v) => !v)}
                    >
                      {isExpanded ? t("casino:show_less") : t("tournament:showMore")}
                    </button>
                  </div>
                )}
              </BonusListHeader>
