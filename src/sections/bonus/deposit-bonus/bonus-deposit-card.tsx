...
export function BonusDepositCard() {
  ...
  if (isAuthLoading) {
    return (
      <div className="relative min-h-[252px] z-0 mx-5 sm:mx-0 sm:absolute sm:w-1/2 sm:right-4 sm:rtl:right-auto sm:rtl:left-4 sm:top-4 sm:bottom-2 sm:h-[calc(100%-2rem)] sm:bg-base-300 sm:rounded-box">
          ...
          <div className="relative w-full">
            ...
            <div className="absolute top-1/2 left-0 right-0 transform -translate-y-1/2 z-0 sm:pr-10 px-4 sm:pl-0">
              <div className="h-1.5 sm:h-2 bg-base-300 rounded-full overflow-hidden relative">
                {/* 已解锁进度条填充 */}
                <div className="h-full bg-primary/20 rounded-full relative overflow-hidden" style={{ width: `${currentProgress}%` }}>
                  {/* 斜条纹动画 - 修复连贯性问题 */}
                  <m.div
