...
const buildProviderAliases = (provider?: string) => {
  ...
  if (normalized === "pragmatic" || normalized === "pragmaticplay" || normalized === "pp") {
    return ["pragmatic", "pragmaticplay", "pp"];
  }
...
export const GameProviderTournaments = ({ provider, withContainer = true, showHeading = true }: GameProviderTournamentsProps) => {
  ...
  if (isLoading && providerTournaments.length === 0) {
    return (
      <section className={sectionClasses}>
        ...
        <div className="h-[300px] bg-base-300/60 rounded-2xl animate-pulse" />
      </section>
    );
  }
        ...
        {showHero ? (
          <>
            ...
            <div className="hidden lg:flex flex-col">
              <div className={cn("relative rounded-t-2xl overflow-hidden", !hasVisual && "bg-base-300")}>
                <div
                ...
                <div className="relative z-10 flex flex-row items-center h-full">
                  <div className="w-[30%] flex flex-col justify-center gap-2">
                    ...
                    <h1 className="text-[40px] font-black text-base-content uppercase leading-[1.05]">
                      <span className="text-primary block">{selectedVisual.titleHighlight}</span>
                  ...
                  <div className="w-[32%]">
                    <div className="bg-base-300/40 rounded-field p-4 mx-auto max-w-sm">
                      <span className="badge badge-success rounded-sm rounded-bl-none badge-xs h-3 text-[9px] font-bold">
                      ...
                      <div className="bg-base-400/50 rounded-field px-4 py-3 mt-3 text-center">
                        <p className="text-xs text-base-content/70 leading-none">{t("tournament:progressivePrizePool")}</p>
                        <p className="text-2xl font-bold text-primary leading-tight mt-1">{formattedPrize.formatted}</p>
                      </div>
                    </div>
                  </div>
                  </div>
            ...
            <div className="lg:hidden flex flex-col">
              <TournamentBanner tournaments={providerTournaments} selectedIndex={selectedIndex} onIndexChange={setSelectedIndex} />
              <TournamentRulesSection tournament={selectedTournament} />
            </div>
          </>
        ) : (
