...
export function TournamentDetailsModal({ isOpen, onClose, tournament, children }: TournamentDetailsModalProps) {
  ...
  if (provider === "jili") {
    titleKey = "jiliTitle";
    descriptionKey = "jiliDescription";
    contentKey = "jiliContent";
  } else if (provider === "pg") {
    titleKey = "pgTitle";
    descriptionKey = "pgDescription";
    contentKey = "pgContent";
  } else if (provider === "pp" || provider === "pragmatic") {
    titleKey = "pragmaticTitle";
    descriptionKey = "pragmaticDescription";
    contentKey = "pragmaticContent";
  } else if (provider === "0" || provider === "newbie") {
    titleKey = "beginnerTitle";
    descriptionKey = "beginnerDescription";
    contentKey = "beginnerContent";
  } else {
    titleKey = safeTournament?.game_provider || "tournament";
    descriptionKey = "beginnerDescription";
    contentKey = "";
  }

  const rulesTitle = String(t(`tournament:${titleKey}`, safeTournament?.name || ""));
  const rulesDescription = String(t(`tournament:${descriptionKey}`));
  const rulesContent = contentKey ? String(t(`tournament:${contentKey}`, "")) : "";
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-stretch gap-4 h-full">
              ...
              <div className="w-full sm:w-[28%] flex items-center justify-center sm:justify-start px-0 sm:px-2">
                <h1 className="text-2xl sm:text-[40px] font-black text-base-content uppercase leading-7 sm:leading-[44px]">
                  <span className="text-primary">{t(visual.titleHighlight, visual.titleHighlight)}</span>
                  <br />
                  {t(visual.title)}
                </h1>
              </div>
                  ...
                  <div className="px-5 sm:px-8 pb-4 space-y-3 text-sm text-base-content/80">
                    ...
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1">
                        <li>{t("tournament:howToParticipateContent.rules1", "Play any participating game during the tournament period")}</li>
                        <li>{t("tournament:howToParticipateContent.rules2", "Each bet contributes to your total wagered amount")}</li>
                        <li>{t("tournament:howToParticipateContent.rules3", "Climb the leaderboard by wagering more")}</li>
                      </ul>
                    </div>
                    ...
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1">
                        <li>{t("tournament:prizeDistributionContent.prize1", "Prizes are distributed based on your final rank")}</li>
                        <li>{t("tournament:prizeDistributionContent.prize2", "Winners will be notified after the tournament ends")}</li>
                        <li>{t("tournament:prizeDistributionContent.prize3", "Prize pool is progressive and increases with participation")}</li>
                      </ul>
                    </div>
                    ...
                    <div>
                      ...
                      <ul className="list-disc list-inside space-y-1">
                        <li>{t("tournament:termsAndConditionsContent.term1", "Only real money bets count towards the tournament")}</li>
                        <li>{t("tournament:termsAndConditionsContent.term2", "Management reserves the right to modify or cancel the tournament")}</li>
                        <li>{t("tournament:termsAndConditionsContent.term3", "General terms and conditions apply")}</li>
                      </ul>
                    </div>
                  </div>
                )}
