...
export const ReferralRatesAndRules = () => {
  ...
  const activeInfographicAlt = selectedReferralType === "indirect"
  ...
  const gameOptions = useMemo<GameSelectOption[]>(() => {
    ...
    return (gameCategoriesData.data as any[])
      ...
      .map((gameType) => {
        const baseLabel = String(t(
          `explore:${gameType.name_key}`,
          gameType.name || gameType.categoryName || gameType.title || "",
        ));
  ...
  const vipOptions = useMemo<VipSelectOption[]>(() => {
    ...
    return (vipConfigData.data as any[]).map((vipLevel) => ({
  ...
  const selectedGameOption = useMemo(() => gameOptions.find((option) => String(option.value) === selectedGameCategory), [gameOptions, selectedGameCategory]);
  const directVipOption = useMemo(() => vipOptions.find((option) => String(option.value) === selectedDirectVip), [vipOptions, selectedDirectVip]);
  const indirectSelfVipOption = useMemo(() => vipOptions.find((option) => String(option.value) === selectedIndirectSelfVip), [vipOptions, selectedIndirectSelfVip]);
  const indirectHighestVipOption = useMemo(() => vipOptions.find((option) => String(option.value) === selectedIndirectHighestVip), [vipOptions, selectedIndirectHighestVip]);
  ...
  const formattedCommissionValue = useMemo(() => {
    const value = commissionDecimal.gt(0) ? commissionDecimal.toFixed(2) : "0.00";
    return formatInputDisplay(value, 2);
  }, [commissionDecimal]);
  ...
  const indirectHighestPercent = indirectHighestVipOption?.spare ? new Decimal(indirectHighestVipOption.spare).mul(100).toFixed(1) : undefined;
  ...
  const showIndirectBreakdown =
    selectedReferralType === "indirect" && indirectSelfPercent && indirectHighestPercent;
  ...
  return (
    ...
    <div className="bg-base-200 rounded-field p-3 sm:p-6 flex flex-col gap-5 w-full">
      ...
      <p className="text-xs sm:text-sm text-base-content/50 block -mt-3">
      {t(
        "referral:calculatorDirectDescription"
      )}
    </p>
)
}
      ...
      <div className="flex flex-col lg:flex-row lg:flex-nowrap gap-3">
        <div className="flex flex-row gap-3 lg:contents">
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-0">
            ...
            <Select
              className={cn("bg-base-300 rounded-field", (isLoadingGameCategories || gameCategoriesError) && "opacity-70 pointer-events-none")}
              dropdownClassName="bg-base-100"
              size="md"
                  ...
                  <div className="flex items-center justify-between gap-3 w-full">
                    ...
                    {gameOption.spare && (
                      <span className="text-xs sm:text-sm font-semibold">
                        ({new Decimal(gameOption.spare).mul(100).toFixed(1)}%)
                      </span>
                    )}
                  </div>
                );
                ...
                renderOption={(option) => {
                  ...
                  return (
                    <p className="text-xs sm:text-sm font-semibold flex items-center justify-between w-full">
                      ...
                      {vipOption.spare && (
                        <span className="text-xs sm:text-sm font-semibold">
                          ({new Decimal(vipOption.spare).mul(100).toFixed(1)}%)
                        </span>
                      )}
                    </p>
                  );
...
{selectedReferralType === "indirect" && (
      ...
      <div className="flex items-center justify-center gap-2">
        <button className="btn btn-ghost min-w-50 flex items-center justify-between" onClick={() => setHideInfographic((prev) => !prev)}>
          {hideInfographic
            ? t("referral:showInfographic", "Show Infographic")
            : t("referral:hideInfographic", "Hide Infographic")}
