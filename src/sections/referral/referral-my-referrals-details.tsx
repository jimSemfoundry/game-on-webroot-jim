...
import { IReferralDetail, ReferralListItem } from '@/types/referral';
import { useTranslation } from 'react-i18next';
import dayjs from 'dayjs';
import { useDisplayCurrencyFormatter } from "@/hooks/currency";


export default function ReferralMyReferralsDetails({ item, isOpen, onClose }: { item: ReferralListItem | null, isOpen: boolean, onClose: () => void }) {
  ...
  useEffect(() => {
    if (item) {
      setLoading(true);
      authService.getReferralDetail({ id: item.id, down_line: item.down_line, refer_type: item.refer_type }).then((res) => {
            ...
            <div className="bg-base-100 rounded-xl p-3">
              <div className="flex flex-col gap-1 py-2">
                ...
                <div className="flex h-6 items-center justify-between px-4">
                  ...
                  <div className="text-sm font-semibold">{dayjs((referralDetail?.regitration_date ?? 0) * 1000).format('YYYY-MM-DD')}</div>
                </div>
                <div className="flex h-6 items-center justify-between px-4">
                ...
                <div className="flex h-6 items-center justify-between px-4">
                  ...
                  <div className="text-sm font-bold">{referralDetail?.refer_type === 'direct' ? t('referral:directfriend') : t('referral:indirectfriend')}</div>
                </div>
                {
                  referralDetail?.refer_type === 'direct' && (
                    <div className="flex h-6 items-center justify-between px-4">
                      ...
                      <div className="text-sm font-bold">
                        {formatWithConversion(referralDetail?.level_reward, "USD", {
                          showSymbol: false,
                          showCode: true,
                        }).formatted}
                      </div>
                ...
                <div className="flex h-6 items-center justify-between px-4">
                  ...
                  <div className="text-sm font-bold text-primary">
                    {formatWithConversion(referralDetail?.group_reward ?? 0, "USD", {
                      showSymbol: false,
                      showCode: true,
                    }).formatted}
                  </div>
