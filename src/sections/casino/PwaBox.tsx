import { isAndroid, isIOS, isMac } from "@/utils/browser";
...
export const PwaBox = () => {
  ...
  const handleInstallClick = () => {
    const promptEvent = (window as unknown as { __pwaPromptEvent?: BeforeInstallPromptEvent }).__pwaPromptEvent;
          ...
          }
          {
              !(isMac() || isIOS()) && (
                   <div className="border border-base-content rounded-xl w-[170px] h-[59px] px-4 flex items-center gap-3" onClick={handleInstallClick}>
                      ...
                      <div className="flex flex-col justify-center">
                           <div className="text-xs font-bold text-base text-base-content leading-3">{t('casino:availableOn')}</div>
                           <div className="text-xl font-bold text-base text-base-content leading-5">Android</div>
                      </div>
                  </div>
              )
          }
          <IoSInstallBox open={openIOS} onClose={() => setOpenIOS(false)} />
      </>
  )
}
