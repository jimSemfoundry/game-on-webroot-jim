...
const REWARD_ROWS: VipRewardRow[] = [
  ...
  {
    icon: "/icons/isometric/vip-monday.png",
    label: "bonus:vip_monday",
    type: "boolean",
    field: "vip_monday",
    getValue: (config: IVipLevelConfig) => {
      return config.vip > 1;
...
export function VipRewardsTableV2() { 
  ...
  const staticColumnCount = isXlUp ? 4 : isMdUp ? 2 : 1;
  ...
  return (
    <div className="rounded-box bg-base-200 overflow-hidden">
      <div className="flex w-full">
        {/* Label Column (auto-width based on content) */}
        <div className="shrink-0">
          ...
          {REWARD_ROWS.map((row, rowIndex) => (
            <InnerGuardContainer item={row} key={`label-${rowIndex}`}>
        ...
        {/* VIP Columns (static + carousel, equal width) */}
        <div className="flex-1 flex min-w-0">
          {/* Static VIP Columns */}
          <div className={cn("grid gap-y-0", "flex-[1] min-[900px]:flex-[2] min-[1280px]:flex-[4]")}>
            <div className={cn("grid", "grid-cols-1 min-[900px]:grid-cols-2 min-[1280px]:grid-cols-4")}>
            {/* Static VIP Headers */}
          ...
          {/* Static VIP Data Rows */}
          {REWARD_ROWS.map((row, rowIndex) => (
            <InnerGuardContainer item={row} key={rowIndex}>
              <Fragment key={rowIndex}>
                <div
                  className={cn("grid", "grid-cols-1 min-[900px]:grid-cols-2 min-[1280px]:grid-cols-4")}>
                  {/* Static Level Cells */}
          ...
          {/* Dynamic Carousel Column */}
          <div className={cn("min-w-0 flex-[1_1_0px]")}>
