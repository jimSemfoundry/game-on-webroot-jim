...
function UnauthenticatedVipHero() {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-box min-h-[500px] sm:min-h-[400px] flex items-start mx-5 sm:mx-0"
      style={{ isolation: "isolate" }}
    >
      <div className="container mx-auto px-0 sm:px-20 py-6 sm:py-12">
        <div className="absolute sm:w-[659px] sm:h-[659px] bg-primary rounded-full blur-[215px] left-[-45px] bottom-0 -translate-y-[268px] -translate-x-full" />
        <div className="flex flex-col sm:flex-row items-center justify-between gap-8">
          <div className="flex-1 z-10">
            <h1 className="text-3xl sm:text-5xl font-bold text-base-content mb-4 whitespace-pre-wrap">{t("vip:exclusive_vip_system")}</h1>
            ...
            <button className="btn btn-primary btn-lg sm:btn-xl text-base ml-2.5 sm:ml-0 px-8 w-[130px]">Participate</button>
          </div>
        </div>

        <img
          src="/images/illustrations/463f8314ce7d0466e5c028410fc517a52b6343e0.png"
          className="w-[114px] h-[114px] sm:w-[252px] sm:h-[252px] rotate-[-15deg] absolute right-[81px] sm:left-auto bottom-[85px] sm:top-[25px] sm:right-[180px] origin-bottom-right z-40 drop-shadow-2xl"
        />

        <img
          src="/images/illustrations/00c1dc7493acb88558ddaacde45133378d09973d.png"
          className="w-[241px] h-[241px] right-[17px] bottom-[0px] translate-y-[100px] sm:translate-y-0 sm:w-[534px] sm:h-[534px] opacity-30 sm:left-auto absolute sm:right-[38px] sm:top-[150px] object-contain"
        />

        <img
          src="/images/illustrations/58f4a6bfd06fc43064eff747c6489124fff520a2.png"
          className="w-[35px] h-[35px] right-[46px] bottom-[115px] sm:w-[78px] sm:h-[78px] absolute sm:left-auto sm:top-[146px] sm:right-[102px] z-40 object-cover"
        />

        <img
          src="/images/illustrations/58f4a6bfd06fc43064eff747c6489124fff520a3.png"
          className="w-[70px] h-[70px] right-[207px] bottom-[45px] sm:w-[155px] sm:h-[155px] absolute sm:left-auto sm:top-[224px] sm:right-[460px] z-40"
        />
        <img
          src="/images/illustrations/44285e1a15e8a0743705e017bbcef43e8bc2546d.png"
          className="w-[41px] h-[41px] bottom-[30px] right-[33px] sm:w-[90px] sm:h-[90px] rotate-[18deg] absolute sm:right-[73px] sm:top-[295px]"
        />
        <div className="absolute hidden sm:block w-[659px] h-[659px] right-[-87px] top-[336px] bg-primary blur-[215px] rounded-full transform-gpu" />
        <div className="absolute block sm:hidden w-[297px] h-[297px] right-[-40px] bottom-0 translate-y-[229px] blur-[97px] bg-primary opacity-50 transform-gpu" />
        <div className="absolute sm:top-[243px] sm:right-[175px] shrink-0 origin-bottom-right text-base-300 hidden sm:block">
...
function AuthenticatedVipHero() {
  ...
  const { formatCurrency, selectedCurrency } = useDisplayCurrency();

  const currentVip = status?.vip || 1;
  const currentXp = Number(status?.xp) || 0;
  ...
  const { data: vipConfigData } = useQuery({
    queryKey: ["vipConfig", currentVip + 1],
    queryFn: async () => {
      const response = await authService.getVipConfig(currentVip + 1);
      console.debug("VipConfig response:", response);
      return response;
    },
    enabled: !!status && currentVip > 0,
  });
  ...
  const nextLevelXp = useMemo(() => {
    if (vipConfigData?.code === 0 && vipConfigData?.data) {
      const nextConfig = Array.isArray(vipConfigData.data) ? vipConfigData.data[0] : vipConfigData.data;
      if (nextConfig?.xp) {
        return Number(nextConfig.xp);
      }
    }
    return 1000;
  }, [vipConfigData]);
  ...
  return (
    <div
      className="relative overflow-visible sm:mt-18 rounded-box sm:bg-base-200/50 min-h-[360px] sm:min-h-[234px] select-none"
      style={{
    ...
    >
      <div className="relative w-full h-full">
        ...
        <div className="flex-1 flex flex-col lg:flex-row items-center lg:items-start gap-4 sm:gap-6 absolute left-[30px] sm:left-[50px] top-1/2 translate-y-1/2">
          {/* Rewards Info */}
          <div className="text-left flex flex-col gap-1 sm:gap-2">
            ...
            <button className="btn btn-sm btn-ghost gap-2 mt-1 flex justify-start px-0">
              <span className="text-sm font-semibold">{t("vip:eligible")}</span>
              <div className="w-4 h-4 rounded-full bg-success flex items-center justify-center text-success-content text-xs font-bold">
                {eligibleCount}
              </div>
        ...
        <div className="w-[225px] h-[169px] right-[-6px] sm:w-[312px] sm:h-[234px] flex items-end justify-center absolute lg:left-[350px] lg:top-0">
          <img
        ...
        {/* 右侧：VIP Progress */}
        <div className="min-h-[200px] top-[167px] left-0 right-0 z-0 mx-5 sm:mx-0 absolute sm:w-[600px] sm:right-4 sm:left-auto sm:rtl:right-auto sm:rtl:left-4 sm:top-4 sm:bottom-2 sm:h-[calc(100%-2rem)] sm:bg-base-300 sm:rounded-box">
          ...
          <div className="relative flex h-full w-full flex-col gap-4 rounded-box p-5 sm:p-6">
            {/* VIP Badge - 右上角 */}
            <div className="absolute top-3 right-6 sm:top-6 sm:right-10 -translate-y-1/2">
              <img
            ...
            <p className="text-sm sm:text-lg text-base-content/50 font-semibold">
              {t("vip:your_vip_progress")}: {xpProgress.toFixed(0)}%
            </p>
            ...
            {/* XP Info */}
            <div className="flex justify-between text-sm sm:text-base text-base-content/50 font-semibold">
              <span>
                {formatXp(xpLeft)} XP {t("vip:left")}
              </span>
              <span>
